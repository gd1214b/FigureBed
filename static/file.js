...
$(() => {
    ...
    function upload(files) {
        ...
        for (let i = 0; i < files.length; i++) {
			...
			var type = $('#type').val();
            //var maxSize = 5242880;
			if(type =='qihoo'|type =='muke'|type =='gtimg'|type =='catbox'|type =='vxichina'|type =='vimcn'|type =='imgbox'|type =='bkimg'){
				var api = 'https://www.hualigs.cn/api/upload?token=8ff730a4dc52360c26854111c794742b&apiType='+$('#type').val();
				var maxSize = 10485760;
			}else if(type =='imgbb'){
				var api = 'https://api.imgbb.com/1/upload?key=f550a1912f29046b64b3272fe6be11df&type='+$('#type').val();
				var maxSize = 33554432;
			}else if(type =='baidu'|type =='douyin'|type =='ks'|type =='lenovo'|type =='mt'|type =='suning'|type =='weidian'){
				var api = 'https://pic.onji.cn/api/'+$('#type').val()+'.php';
				var maxSize = 5242880;
			}else{
				var api = 'https://image.kieng.cn/upload.html?type='+$('#type').val();
				var maxSize = 5242880;}
            ...
            $.ajax({
                ...
                success: res => {
                    if(type =='muke'){var imgSrc = res.data.url.muke}else if(type =='vxichina'){var imgSrc = res.data.url.vxichina}else if(type =='qihoo'){ var imgSrc = res.data.url.qihoo}else if(type =='catbox'){var imgSrc = res.data.url.catbox}else if(type =='gtimg'){var imgSrc = res.data.url.gtimg}else if(type =='vimcn'){var imgSrc = res.data.url.vimcn}else if(type =='imgbox'){var imgSrc = res.data.url.imgbox}else if(type =='bkimg'){var imgSrc = res.data.url.bkimg}else{var imgSrc = res.data.url}
                    /* 清除input框 */
                    $('#file').val(null);
                    if (res.code === -1) {
                        $('.' + randomClass).fadeOut();
